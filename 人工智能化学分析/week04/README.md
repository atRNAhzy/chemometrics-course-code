...
本目录对应《化学自动化》课程第 4 次作业，主题是用机器学习预测有机酸分子的 `pKa`。
...
conda activate aichem_week05
pip install -r week04/requirements.txt
jupyter notebook
```

然后打开：

`week04/pKa_prediction_experiment.ipynb`

按顺序运行全部单元格即可。

Notebook 默认会：

- 从 `week04/data/` 读取训练集和测试集
- 把结果文件写到 `week04/results/`
